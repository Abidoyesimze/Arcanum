...
import { Download, Lock, AlertCircle, CheckCircle, ArrowRight, Info, Zap, Search } from "lucide-react"
...
export default function DepositPage() {
  ...
  const { contract: vaultContract } = useContract({
    abi: VaultManagerContract.abi as Abi,
    address: VaultManagerContract.address as `0x${string}`,
  })

  const { sendAsync: sendApproval } = useSendTransaction({})
  const { sendAsync: sendDeposit } = useSendTransaction({})
  ...
  const availableTokens: Token[] = [
    {
      ...
      symbol: "ETH",
      name: "Ethereum",
      balance: ethBalance ? Number.parseFloat(formatTokenAmount(ethBalance as any, 18)) : 0,
      balanceUSD: ethBalance ? Number.parseFloat(formatTokenAmount(ethBalance as any, 18)) * 1700 : 0,
      icon: "Ξ",
    ...
    },
    {
      ...
      symbol: "USDC",
      name: "USD Coin",
      balance: usdcBalance ? Number.parseFloat(formatTokenAmount(usdcBalance as any, 6)) : 0,
      balanceUSD: usdcBalance ? Number.parseFloat(formatTokenAmount(usdcBalance as any, 6)) : 0,
      icon: "$",
    ...
    },
    {
      ...
      symbol: "USDT",
      name: "Tether",
      balance: usdtBalance ? Number.parseFloat(formatTokenAmount(usdtBalance as any, 6)) : 0,
      balanceUSD: usdtBalance ? Number.parseFloat(formatTokenAmount(usdtBalance as any, 6)) : 0,
      icon: "₮",
      contractAddress: TOKEN_ADDRESSES.USDT,
      decimals: 6,
    },
    {
      id: "4",
      symbol: "DAI",
      name: "Dai Stablecoin",
      balance: daiBalance ? Number.parseFloat(formatTokenAmount(daiBalance as any, 18)) : 0,
      balanceUSD: daiBalance ? Number.parseFloat(formatTokenAmount(daiBalance as any, 18)) : 0,
      icon: "◈",
      contractAddress: TOKEN_ADDRESSES.DAI,
      decimals: 18,
    },
  ]
  ...
  useEffect(() => {
    if (selectedToken && depositAmount && allowance) {
      const amountBigInt = parseTokenAmount(depositAmount, selectedToken.decimals)
      const allowanceBigInt =
        BigInt((allowance as any).low || 0) + (BigInt((allowance as any).high || 0) << BigInt(128))
  ...
  const depositSteps: DepositStep[] = [
    ...
    },
    {
      ...
      isComplete: depositAmount !== "" && Number.parseFloat(depositAmount) > 0,
  ...
  const handleApprove = async () => {
    if (!selectedToken || !tokenContract || !address) return

    setIsApproving(true)
    try {
      const amountBigInt = parseTokenAmount(depositAmount, selectedToken.decimals)
      const amountU256 = toU256(amountBigInt)

      const approveCall = tokenContract.populate("approve", [VaultManagerContract.address, amountU256])

      await sendApproval([approveCall])
  ...
  const handleDeposit = async () => {
    if (!selectedToken || !depositAmount || !vaultContract || !address) return
    ...
    try {
      ...
      const depositCall = vaultContract.populate("deposit_to_vault", [
        selectedToken.contractAddress,
        amountU256,
        commitment,
      ])

      await sendDeposit([depositCall])
        ...
        <React.Fragment key={step.step}>
          <div className={`flex flex-col items-center ${step.isActive ? "z-10" : ""}`}>
            ...
            <div className="text-center mt-2 max-w-[120px]">
              <p className={`text-xs font-medium ${step.isActive ? "text-white" : "text-white/70"}`}>{step.title}</p>
            </div>
          </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0A0118] via-[#1A0B2E] to-[#0A0118] p-4 lg:p-6 flex items-center justify-center">
        <div className="bg-glass rounded-2xl p-8 text-center max-w-md">
          <Lock className="w-16 h-16 text-encrypted mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-white mb-4">Connect Your Wallet</h2>
          <p className="text-gray-300 mb-6">Please connect your Starknet wallet to make deposits</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {depositSuccess && selectedToken && (
            <div className="bg-glass rounded-2xl p-8 text-center">
              ...
              <h2 className="text-3xl font-bold text-white mb-4 font-space-grotesk">Deposit Successful!</h2>
              ...
              <div className="bg-encrypted/10 border border-encrypted/30 rounded-xl p-6 mb-8 max-w-md mx-auto">
                {privacyMode ? (
                  <p className="text-encrypted font-bold text-2xl font-jetbrains mb-2">●●●●●● {selectedToken.symbol}</p>
                ) : (
              ...
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-6 mb-8">
                ...
                <div className="space-y-3 text-left">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-blue-400 rounded-full flex-shrink-0"></div>
                    <p className="text-white/70 text-sm">Zero-knowledge proof generated and verified</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-blue-400 rounded-full flex-shrink-0"></div>
                    <p className="text-white/70 text-sm">Balance hidden with cryptographic commitment</p>
                  </div>
                </div>
              </div>
          ...
          {currentStep === 1 && !depositSuccess && (
            <div className="bg-glass rounded-2xl p-8">
              <h2 className="text-2xl font-bold text-white mb-6 font-space-grotesk">Select Token to Deposit</h2>
                      ...
                      <div className="text-right">
                        {privacyMode ? (
                          ...
                          <>
                            ...
                            <p className="text-gray-400 text-sm">${token.balanceUSD.toLocaleString()}</p>
                          </>
                        )}
                      </div>
                    </div>
          ...
          {currentStep === 2 && selectedToken && !depositSuccess && (
            <div className="bg-glass rounded-2xl p-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white font-space-grotesk">Enter Deposit Amount</h2>
                <button
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-400 text-sm">
                      Available: {privacyMode ? "●●●●●●" : `${selectedToken.balance} ${selectedToken.symbol}`}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                {depositAmount && (
                  <p className="text-gray-400 text-sm mt-2">
                    ≈ $
                    {(Number.parseFloat(depositAmount) * (selectedToken.balanceUSD / selectedToken.balance)).toFixed(2)}{" "}
                    USD
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-4 gap-2 mb-6">
                {["25", "50", "75", "100"].map((percent) => (
                  <button
                    key={percent}
                    onClick={() =>
                      setDepositAmount(((selectedToken.balance * Number.parseInt(percent)) / 100).toString())
                    }
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/70 text-sm mt-1">
                      Your deposit amount will be hidden using zero-knowledge proofs. Only you can see your balance.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                disabled={
                  ...
                  Number.parseFloat(depositAmount) > selectedToken.balance
                }
          ...
          {currentStep === 3 && selectedToken && !depositSuccess && (
            <div className="bg-glass rounded-2xl p-8">
              <h2 className="text-2xl font-bold text-white mb-6 font-space-grotesk">Generate Zero-Knowledge Proof</h2>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-white">
                      $
                      {(Number.parseFloat(depositAmount) * (selectedToken.balanceUSD / selectedToken.balance)).toFixed(
                        2,
                      )}
                    </span>
                  </div>
              ...
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-6 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-blue-400 font-medium text-sm mb-2">What is a Zero-Knowledge Proof?</p>
                    <p className="text-white/70 text-sm">
                      A cryptographic proof that verifies you have sufficient funds without revealing the exact amount.
                      This keeps your balance completely private while allowing you to interact with DeFi protocols.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {currentStep === 4 && selectedToken && !depositSuccess && (
            <div className="bg-glass rounded-2xl p-8">
              <h2 className="text-2xl font-bold text-white mb-6 font-space-grotesk">Confirm Deposit</h2>
              ...
              <div className="bg-warning-amber/10 border border-warning-amber/30 rounded-lg p-4 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/70 text-sm mt-1">
                      Once deposited, your balance will be hidden. Only you can view it with your private key.
                    </p>
                  </div>
                </div>
              </div>
